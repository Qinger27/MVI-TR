...
class ResNet50(nn.Module):
    def __init__(self, num_class=2):
        ...
        # 使用卷积代替全连接
        self.conv11 = nn.Conv2d(2048, num_class, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)  # [8, 2048, 7, 7]
        # print("-------1", out.shape)
        out = self.avgpool(out)      # [8, 2048, 1, 1]
        # print("-------2", out.shape)
        out = self.conv11(out)      # [8, 2, 1, 1]
        # print("-------3", out.shape)
        out = out.reshape(x.shape[0], -1)    # [8, 2]
        # print("-------4", out.shape)
        # out = self.fc(out)
        return out
